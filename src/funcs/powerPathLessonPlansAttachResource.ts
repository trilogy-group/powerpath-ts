 ...
 */
export function powerPathLessonPlansAttachResource(
  client: PowerPathCore,
  request: operations.AttachResourceRequest,
  options?: RequestOptions,
): APIPromise<
  Result<
    operations.AttachResourceResponse,
...
> {
  const parsed = safeParse(
    request,
    (value) => operations.AttachResourceRequest$outboundSchema.parse(value),
  ...
  const path = pathToFunc("/powerpath/lessonPlans/{lessonPlanId}/resource")(
    pathParams,
  );
  ...
  const [result] = await M.match<
    ...
    | SDKValidationError
  >(
    M.json(201, operations.AttachResourceResponse$inboundSchema),
