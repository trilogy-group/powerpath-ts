 ...
 */
export function powerPathLessonPlansAttachComponent(
  client: PowerPathCore,
  request: operations.AttachComponentRequest,
  options?: RequestOptions,
): APIPromise<
  Result<
    operations.AttachComponentResponse,
...
> {
  const parsed = safeParse(
    request,
    (value) => operations.AttachComponentRequest$outboundSchema.parse(value),
  ...
  const pathParams = {
    lessonPlanId: encodeSimple("lessonPlanId", payload.lessonPlanId, {
      explode: false,
      charEncoding: "percent",
    }),
  };

  const path = pathToFunc("/powerpath/lessonPlans/{lessonPlanId}/component")(
    pathParams,
  );
  ...
  const [result] = await M.match<
    ...
    | SDKValidationError
  >(
    M.json(201, operations.AttachComponentResponse$inboundSchema),
