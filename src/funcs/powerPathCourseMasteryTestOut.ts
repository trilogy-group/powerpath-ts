 ...
 */
export function powerPathCourseMasteryTestOut(
  client: PowerPathCore,
  request: operations.TestOutRequest,
  options?: RequestOptions,
): APIPromise<
  Result<
    components.TestOutResult,
...
> {
  const parsed = safeParse(
    request,
    (value) => operations.TestOutRequest$outboundSchema.parse(value),
  ...
  const path = pathToFunc("/powerpath/testOut")();
  ...
  const [result] = await M.match<
    ...
    | SDKValidationError
  >(
    M.json(200, components.TestOutResult$inboundSchema),
