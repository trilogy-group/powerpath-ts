 ...
 * Get course syllabus
 */
export function lessonPlansGetSyllabus(
  client: PowerPathCore,
  request: operations.GetCourseSyllabusRequest,
  options?: RequestOptions,
): APIPromise<
  Result<
    operations.GetCourseSyllabusResponse,
...
> {
  const parsed = safeParse(
    request,
    (value) => operations.GetCourseSyllabusRequest$outboundSchema.parse(value),
  ...
  const path = pathToFunc("/powerpath/syllabus/{courseSourcedId}")(pathParams);
  ...
  const [result] = await M.match<
    ...
    | ConnectionError
  >(
    M.json(200, operations.GetCourseSyllabusResponse$inboundSchema),
