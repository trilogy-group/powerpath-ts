  ...
  implements SDKInitHook, BeforeRequestHook, AfterErrorHook
{
  ...
  private async getCredentialsGlobal(
    ...
    const envTokenURL = env().POWERPATH_TOKEN_URL ?? DEFAULT_TOKEN_URL;
    ...
    return {
      clientID: out?.clientID ?? env().POWERPATH_CLIENT_ID ?? "",
      clientSecret: out?.clientSecret ?? env().POWERPATH_CLIENT_SECRET ?? "",
